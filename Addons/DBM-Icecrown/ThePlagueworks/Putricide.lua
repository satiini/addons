...
function mod:SPELL_CAST_START(args)
	if args.spellId == 70351 then
		...
		if glime then
			sndWOP:Schedule(3, "Interface\\AddOns\\DBM-Core\\extrasounds\\gslime.mp3")
			sndWOP:Schedule(7, "Interface\\AddOns\\DBM-Core\\extrasounds\\killslime.mp3")
			glime = false
		else
			sndWOP:Schedule(3, "Interface\\AddOns\\DBM-Core\\extrasounds\\rslime.mp3")
			sndWOP:Schedule(7, "Interface\\AddOns\\DBM-Core\\extrasounds\\killslime.mp3")
			glime = true
		end
	elseif args:IsSpellID(71617) then				--Tear Gas, normal phase change trigger
		...
		if mod:IsMelee() then
			sndWOP:Cancel("Interface\\AddOns\\DBM-Core\\extrasounds\\bombsoon.mp3")
		end
...
function mod:NextPhase()
	...
	if phase == 2 then
		...
		if mod:IsMelee() then
			sndWOP:Schedule(12, "Interface\\AddOns\\DBM-Core\\extrasounds\\bombsoon.mp3")
		end
		if self:IsDifficulty("heroic10", "heroic25") then
	...
	elseif phase == 3 then
		...
		if mod:IsMelee() then
			sndWOP:Schedule(9, "Interface\\AddOns\\DBM-Core\\extrasounds\\bombsoon.mp3")
		end
		if self:IsDifficulty("heroic10", "heroic25") then
			timerUnboundPlagueCD:Start(50)
		end
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args.spellId == 71255 then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\gasbomb.mp3")
		if mod:IsMelee() then
			sndWOP:Schedule(32, "Interface\\AddOns\\DBM-Core\\extrasounds\\bombsoon.mp3")
		end
	...
	elseif args.spellId == 72295 then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\greenball.mp3")
		if self:IsDifficulty("heroic10", "heroic25") then
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 70352 then	--Ooze Variable
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\gslime.mp3")
		end
	elseif args.spellId == 70353 then	-- Gas Variable
		if args:IsPlayer() then
			specWarnGasVariable:Show()
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\rslime.mp3")
		end
	elseif args.spellId == 70911 then	 -- Unbound Plague
		...
		if args:IsPlayer() then
			...
			sndWOP:Schedule(30, "Interface\\AddOns\\DBM-Core\\extrasounds\\transplague.mp3")
		end
	end
end
...
function mod:SPELL_AURA_REMOVED(args)
	...
	elseif args.spellId == 70911 then 						-- Unbound Plague
		...
		if args:IsPlayer() then
			sndWOP:Cancel("Interface\\AddOns\\DBM-Core\\extrasounds\\transplague.mp3")
		end
