...
function mod:OnCombatStart(delay)
	...
	if self:IsDifficulty("normal10", "heroic10") then
		...
		sndWOP:Schedule(115, "Interface\\AddOns\\DBM-Core\\extrasounds\\ptwo.mp3")
	else
		timerNextInciteTerror:Start(127-delay)
		sndWOP:Schedule(120, "Interface\\AddOns\\DBM-Core\\extrasounds\\ptwo.mp3")
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args.spellId == 70877 then
		...
		if args:IsPlayer() then
			...
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\bitethem.mp3")
			if self:IsDifficulty("normal10", "heroic10") then
	...
	elseif args:IsSpellID(70867, 70879) then	--Essence of the Blood Queen
		...
		if args:IsPlayer() then
			...
			if self:IsDifficulty("normal10", "heroic10") then
				...
				sndWOP:Schedule(70, "Interface\\AddOns\\DBM-Core\\extrasounds\\bitesoon.mp3")
			else
				timerEssenceoftheBloodQueen:Start()--60 seconds on 25 man
				warnBloodthirstSoon:Schedule(55)
				sndWOP:Schedule(55, "Interface\\AddOns\\DBM-Core\\extrasounds\\bitesoon.mp3")
			end
		end
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 73070 then				--Incite Terror (fear before air phase)
		...
		if self:IsDifficulty("normal10", "heroic10") then
			...
			sndWOP:Schedule(112, "Interface\\AddOns\\DBM-Core\\extrasounds\\ptwo.mp3")
		else
			timerNextInciteTerror:Start()--100 seconds in between first and second on 25 man
			sndWOP:Schedule(92, "Interface\\AddOns\\DBM-Core\\extrasounds\\ptwo.mp3")
		end
	end
end
