...
function mod:SPELL_CAST_START(args)
	...
	elseif args.spellId == 72039 then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\powervortex.mp3")
	...
	elseif args.spellId == 72040 then	-- Conjure Empowered Flames
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\powerflame.mp3")
		timerConjureFlamesCD:Start()
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	if args.spellId == 70952 then
		...
		if self:IsInCombat() then
			...
			sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\changetarget.mp3")
...
function mod:OnSync(msg)
	if msg == "KineticBomb" then
		...
		sndWOP:Play("Interface\\AddOns\\DBM-Core\\extrasounds\\bombsoon.mp3")
	end
end
